...
PVector nndrorigin = new PVector(60,50);
...
class Drawit {
  ...
  byte [] getBytes() {
    ...
    //for (int x=0; x<nCellsX; x++)for (int y=0; y<nCellsY; y++) text(r[x+(nCellsX*y)],x*sCell+origin.x+(sCell/2),y*sCell+origin.y+(sCell/2));
    fill(255);
    byte [] r2= new byte [(nPixX*nPixY)+inImgb];
    for (int i=0; i<inImgb; i++) r2[i]= 0; 
    for (int i=0; i<r2.length-inImgb; i++) r2[i+inImgb]= r[i]; 
    return r2;
  }
}